...
const FreeWarPopup = ({ isOpen, onClose, onJoinNow }: FreeWarPopupProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md md:max-w-lg p-0 overflow-hidden border-2 border-orange-500">
        <div className="bg-gradient-to-br from-[#1A2A44] to-[#F97316] text-white">
          <DialogHeader className="p-6 space-y-3">
            ...
            <DialogTitle className="text-2xl text-center font-bold text-white drop-shadow-md">
          ...
          <div className="space-y-6 p-6">
            ...
            <div className="space-y-2 p-4 bg-white/10 backdrop-blur-sm rounded-md">
              ...
              <p className="font-medium">Win from a total <span className="text-orange-300 font-bold">₹500 prize pool</span></p>
            </div>
            
            <p className="text-center italic text-sm text-white/80">No entry fee. No catch. Just your cricket brain.</p>
            
            <div className="space-y-2">
              <p className="font-semibold text-center">Prize Pool Breakdown:</p>
              <div className="grid grid-cols-2 gap-3 px-4 py-3 bg-white/10 backdrop-blur-sm rounded-md">
          ...
          <div className="flex justify-center space-x-4 p-6 bg-[#1A2A44]/50">
            <Button 
              variant="outline" 
              size="lg" 
              onClick={onClose}
              className="px-6 py-2 h-12 font-medium bg-white/10 backdrop-blur-sm text-white border-white/20 hover:bg-white/20"
            >
              Dismiss
            </Button>
            <Button 
              ...
              size="lg" 
              onClick={onJoinNow}
              className="px-8 py-2 h-12 font-semibold bg-gradient-to-r from-orange-500 to-orange-400 hover:brightness-110 animate-pulse-subtle shadow-lg border-2 border-orange-300/20"
            >
              Earn Now
            </Button>
          </div>
