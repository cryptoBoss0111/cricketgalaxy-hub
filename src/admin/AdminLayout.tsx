...
const AdminLayout = ({
  ...
  const sidebarItems: SidebarItem[] = [{
    icon: <LayoutDashboard size={20} className="bg-gray-400" />,
    label: 'Dashboard',
    href: '/admin/dashboard',
    active: location.pathname === '/admin/dashboard'
  }, {
    icon: <FileText size={20} className="bg-gray-500" />,
    label: 'Articles',
    href: '/admin/articles',
    active: location.pathname.includes('/admin/articles')
  }, {
    icon: <Layers size={20} className="bg-zinc-500" />,
    label: 'Hero Slider',
    href: '/admin/hero-slider',
    active: location.pathname === '/admin/hero-slider'
  }, {
    icon: <TrendingUp size={20} className="bg-gray-500" />,
    label: 'Top Stories',
    href: '/admin/top-stories',
    active: location.pathname === '/admin/top-stories'
  }, {
    icon: <PieChart size={20} className="bg-gray-500" />,
    label: 'Fan Polls',
    href: '/admin/fan-polls',
    active: location.pathname === '/admin/fan-polls'
  }, {
    icon: <Navigation size={20} className="bg-gray-500" />,
    label: 'Navigation',
    href: '/admin/navigation',
    active: location.pathname === '/admin/navigation'
  }, {
    icon: <Award size={20} className="bg-gray-500" />,
    label: 'Fantasy Picks',
    href: '/admin/fantasy-picks',
    active: location.pathname === '/admin/fantasy-picks'
  }, {
    icon: <Calendar size={20} className="bg-gray-500" />,
    label: 'Match Schedule',
    href: '/admin/matches',
    active: location.pathname === '/admin/matches'
  }, {
    icon: <Users size={20} className="bg-gray-500" />,
  ...
  return <div className={cn("min-h-screen bg-gray-50", isCollapsed && !isMobile ? "admin-collapsed" : "")}>
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-40 bg-gradient-to-b from-cricket-primary to-cricket-secondary text-white transition-all", isMobile ? "transition-transform" : "", isMobile && !isMobileMenuOpen ? "-translate-x-full" : "", isCollapsed && !isMobile ? "w-20" : "w-64")}>
        <div className="p-4 flex items-center justify-between bg-zinc-50">
          {!isCollapsed && <Link to="/admin/dashboard">
              <span className="text-xl font-heading font-bold text-zinc-950">
                Admin Panel
              </span>
            </Link>}
          
          {!isMobile && <Button variant="ghost" size="icon" onClick={() => setIsCollapsed(!isCollapsed)} className="hover:bg-white/10 text-gray-950">
            ...
            <nav className="space-y-1">
              {sidebarItems.map((item, index) => <Tooltip key={index} delayDuration={0}>
                  <TooltipTrigger asChild>
                    <Link to={item.href} className={cn("flex items-center py-3 px-3 rounded-md text-sm font-medium transition-colors", item.active ? "bg-white/20 text-white" : "text-white/80 hover:text-white hover:bg-white/10", isCollapsed ? "justify-center" : "")}>
                      <span className={isCollapsed ? "" : "mr-3"}>{item.icon}</span>
                      {!isCollapsed && <span className="text-zinc-950">{item.label}</span>}
                    </Link>
                  </TooltipTrigger>
        ...
        <div className="mt-auto p-4">
          ...
          <TooltipProvider>
            <div className="flex items-center">
              {!isCollapsed && adminUser && <Avatar className="h-10 w-10 mr-3 bg-white/20 text-white">
                  <span className="text-sm font-semibold">{adminUser.username?.substring(0, 2).toUpperCase() || 'AD'}</span>
                </Avatar>}
              ...
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={handleLogout} className="text-white hover:text-white hover:bg-white/10">
                    <LogOut size={20} />
                  </Button>
                </TooltipTrigger>
          ...
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild>
              <Link to="/" className={cn("flex items-center py-2 px-3 rounded-md text-sm font-medium text-white/80 hover:text-white hover:bg-white/10 mt-4 transition-colors", isCollapsed ? "justify-center" : "")}>
      ...
      <main className={cn("transition-all duration-200", isMobile ? "ml-0 pt-16" : "", isCollapsed && !isMobile ? "ml-20" : isMobile ? "ml-0" : "ml-64")}>
        {children}
